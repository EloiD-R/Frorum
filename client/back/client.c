...
void sendMessage(int clientSocket){
    ...
    scanf("%c", &buffer);
    int sent_message = send(clientSocket, message, strlen(message), 0);

    if(sent_message == -1){
        writeLog(sent_messagSe, "Error while sending", clientSocket, 1);
    } else{
        writeLog(sent_message, "Message sent successfully to server", clientSocket, 2);
    }
}
...
void* receiveMessage(void* arg){
    ...
    ssize_t receiving_message_return = recv(socket, buffer, sizeof(buffer), 0);
